...
} from '../../billing-invoices/billing-invoices.types';
...
import {IPukalPaymentFilter} from '../billing-pukal-payment.types';
import {PUKAL_STATUS, PUKAL_TYPE} from '../billing-pukal-payment.constants';
import {BillingPukalPaymentModal} from './billing-pukal-payment-modal';

function PukalPaymentFilter(props: {
  onSearch: (values: IPukalPaymentFilter) => void;
  currentFilters?: IPukalPaymentFilter;
}) {
...
export const BillingPukalPaymentListing = () => {
  const [filters, setFilters] = React.useState<IPukalPaymentFilter>({});
  const paginationState = usePaginationState();
  const [showEditModal, setShowEditModal] = React.useState(false);

  const onSearch = (newFilters: IPukalPaymentFilter) => {
  ...
  return (
    <PageContainer
      ...
      />
      <HasPermission accessWith={[billingPukalPaymentRole.view]}>
        <PukalPaymentFilter onSearch={onSearch} currentFilters={filters} />
